from flask import Blueprint, jsonify, request

from ..core.utils import get_logger
from ..database.repository import fetch_anomaly_events, fetch_freshness_status
from ..schemas.anomaly_schema import build_anomaly_payload
from ..schemas.common_schema import build_error_response, build_success_response
from .validators import parse_bool, parse_limit_offset, parse_source, parse_symbol, parse_time_range

anomalies_bp = Blueprint("anomalies", __name__, url_prefix="/api")
logger = get_logger("backend.api.anomalies")


@anomalies_bp.get("/anomalies")
def get_anomalies():
    try:
        limit, offset = parse_limit_offset(
            request.args.get("limit"),
            request.args.get("offset"),
            default_limit=50,
            max_limit=5000,
        )
        symbol = parse_symbol(request.args.get("symbol"))
        source = parse_source(request.args.get("source"))
        anomalies_only = parse_bool(
            request.args.get("anomalies_only"),
            default=True,
            field_name="anomalies_only",
        )
        start_time, end_time = parse_time_range(
            request.args.get("start_time"),
            request.args.get("end_time"),
        )
        records = fetch_anomaly_events(
            limit=limit + 1,
            offset=offset,
            symbol=symbol,
            source=source,
            anomalies_only=bool(anomalies_only),
            start_time=start_time,
            end_time=end_time,
        )
        has_more = len(records) > limit
        records = records[:limit]
        payload = build_anomaly_payload(records)
        response = build_success_response(
            data=payload,
            source="processing",
            filters={
                key: value
                for key, value in {
                    "symbol": symbol,
                    "source": source,
                    "anomalies_only": anomalies_only,
                    "start_time": start_time,
                    "end_time": end_time,
                }.items()
                if value is not None
            },
            pagination={"limit": limit, "offset": offset, "has_more": has_more},
            freshness=fetch_freshness_status(source=source),
            no_data=len(records) == 0,
        )
        return jsonify(response), 200
    except ValueError as exc:
        return jsonify(build_error_response("invalid_input", str(exc))), 400
    except Exception as exc:
        logger.exception("Failed to fetch anomalies")
        return jsonify(
            build_error_response(
                "repository_error",
                "Failed to fetch anomaly events.",
                {"reason": str(exc)},
            )
        ), 500
